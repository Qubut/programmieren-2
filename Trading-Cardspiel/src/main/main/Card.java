...
public class Card {
    ...
    public Card(String name, int baseDefense, int baseAttack, Category category, int health) {
        if (name != null && !name.isEmpty())
            this.name = name;
        else throw new IllegalArgumentException("Name can't be null or empty");
        if (baseDefense >= 0)
            this.baseDefense = baseDefense;
        else throw new IllegalArgumentException("Base defense can not be less than 0");
        if (baseAttack >= 0)
            this.baseAttack = baseAttack;
        else throw new IllegalArgumentException("Attack defense can not be less than 0");
        this.category = category;
        if (health >= 0)
            this.health = health;
        else throw new IllegalArgumentException("Health can not be less than 0");
        this.buff = null;
    }
    ...
    public int getDefense() {
        int value = getBaseDefense() + getBuff().getDefenseBuff();
        return (getBuff() != null) ? formatValue(value) : getBaseDefense();
    }

    public int getAttack() {
        int value = getBaseAttack() + getBuff().getAttackBuff();
        return (getBuff() != null) ? formatValue(value): getBaseAttack();
    }
/**
 * <p>deducts from the other card if the defense less than the attack</p>
 * <p>deducts 1 point if the defense is equal to the attack</p>
 * <p> does nothing if the defense more than the attack</p> 
 ...
 */
    public void attack(Card otherCard) {
        if (otherCard != null) {
            ...
            } else throw new IllegalStateException("Creature must be alive");
        } else throw new IllegalArgumentException("card cannot be null");
    }
